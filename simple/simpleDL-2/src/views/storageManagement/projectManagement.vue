      ...
      <el-table-column :label="$t('views.storageManagement.storage.storageName')">
        <template slot-scope="item">
          <router-link class="link"
                       :to="{name:'proStorageDetail',query:{id:item.row.id, is_init: tableData.sel.is_init}}">{{item.row.name}}</router-link>
        </template>
      </el-table-column>
    ...
    <el-dialog :title="$t('views.storageManagement.storage.btnCreatePro')"
      ...
      <div>
        <div class='create-storage'>
          <span>{{$t('views.storageManagement.storage.spaceName')}}
            <el-tooltip content="bottom-start"
                        placement="bottom-start">
              <i class="iconfont icon-Group"></i>
            </el-tooltip>
          </span>
          <el-input v-model="createStorage.storage_name"></el-input>
        </div>
        <div class='create-storage'>
          <span>所属项目空间</span>
...
export default {
  ...
  methods: {
    ...
    saveStorage () {
      ...
      let parms = {is_init: this.tableData.sel.is_init, name: this.createStorage.storage_name, project_id: this.createStorage.project_id, describe: this.createStorage.storage_description}
    ...
    saveAuthenticate (row) {
      ...
      link.href = process.env.API_HOST + '/v1/storage/downloadkey?access-token=' + JSON.parse(localStorage.getItem('userinfo')).access_token + '&storage_id=' + row.id
  ...
  created () {
    // 每有一个表格就要写个初始化赋值
    this.$get('/storage/progectlist').then(res => {
      if (res.code === 200) {

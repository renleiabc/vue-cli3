    ...
    <div class="views-research-forum-management">
        <div class="tab-select">
            <div>
                ...
                <el-input v-model="obj.title" :placeholder='$t("views.researchForum.enter")'></el-input>
            </div>
            <div>
                <span class="tab-select-title">{{$t("views.researchForum.type")}}</span>
                <el-select v-model="obj.type" :placeholder='$t("views.researchForum.chose")' clearable>
                    <el-option v-for="item in forumaddper" :key="item.value" :label="item.label" :value="item.value">
                    </el-option>
                </el-select>
            </div>
            <div>
                <span class="tab-select-title">{{$t("views.researchForum.poster")}}</span>
                <el-input v-model="obj.name" :placeholder='$t("views.researchForum.enter")'></el-input>
            </div>
            <div>
                <el-button class="btn light small" @click="clearSearch">{{$t("views.researchForum.reset")}}</el-button>
                <el-button class="btn light small" @click="doSearch">{{$t("views.researchForum.select")}}</el-button>
            </div>
        </div>
        <div class="else-do">
            <el-button class="btn dark mid" @click="deleteForum">{{$t("views.researchForum.batchDeletion")}}</el-button>
        ...
        <div class="views-forum-table">
            <el-table class="tab-content" :data="forumData" @selection-change="selectChange" v-loading="loading" border @sort-change="sortChange">
                <el-table-column type="selection" width="55" align="center"></el-table-column>
                <el-table-column :label='$t("views.researchForum.theme")' align="center">
                    <template slot-scope="scope">
                        <router-link class="link" :to="{name:'forumDetails',query:{forumId:scope.row.id}}">{{scope.row.title}}</router-link>
                    </template>
                </el-table-column>
                <el-table-column :label='$t("views.researchForum.postType")' width="80" align="center">
                ...
                <el-table-column prop="user_name" :label='$t("views.researchForum.poster")' align="center"></el-table-column>
                <el-table-column :label='$t("views.researchForum.replyView")' align="center" prop="views" sortable='custom'>
                    <template slot-scope="item">
                        {{item.row.reply_num}}/{{item.row.views}}
                    </template>
                </el-table-column>
                <el-table-column :label='$t("views.researchForum.finalPost")' align="center" prop="last_at" sortable='custom'>
                    <template slot-scope="item">
                ...
                <el-table-column :label='$t("views.researchForum.operating")' width="100" align="center">
                    <template slot-scope="scope">
                        <button class="btn" @click="deleteSingle(scope.row.id)">{{$t("views.researchForum.delete")}}</button>
                    </template>
                </el-table-column>
                <!--暂无数据情况-->
                <div slot="empty">

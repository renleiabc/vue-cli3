...
<template>
    <div class="views-research-forum-my">
        <el-tabs v-model="activeName" @tab-click="searchTabs">
            <el-tab-pane :label='item.label' :name="item.name" v-for="(item,index) in tag" :key="index">
                <div class="tab-select">
                    <div>
                        <el-input :placeholder='$t("views.researchForum.enter")' v-model="obj.keyword" clearable>
                ...
                <!-- 选择不同的类型 -->
                <p class="sel">
                    <button v-for="(item,index) in type" :class="{active:item.active}" @click="changeType(index)" :key="index">
                        {{item.label}}
                    </button>
                </p>
                    ...
                    <el-table-column :label='$t("views.researchForum.postTopic")'>
                        <template slot-scope="scope">
                            <router-link class="link" :to="{name:'forumDetails',query:{forumId:scope.row.id}}">
                                <i :class="scope.row.type==='1'?'icon-qiuzhu':scope.row.type==='2'?'icon-fenxiang':scope.row.type==='3'?'icon-tongzhi1':'icon-gonggao2'" class="icon iconfont"></i>
                                {{scope.row.title}}
                            </router-link>
                        </template>
                    </el-table-column>
                ...
                <el-pagination class="tab-page" layout="prev, pager, next" :page-size="per" :current-page.sync="currentPage" :total="total" @current-change="changePageTabs" v-show='isCount'></el-pagination>
            </el-tab-pane>
        </el-tabs>
    </div>
</template>

...
<template>
    <div class="views-research-forum-details">
        <p class="forum-title">
            {{forum.title}}
            <el-button class="btn dark small" @click="showReplyPop">{{$t("views.researchForum.reply")}}</el-button>
        </p>
        <div class="content">
            <div class="title">
                <div>
            ...
            <div class="msg" v-html="forum.content"></div>
        </div>
        <!-- 帖子回复 -->
        <ul class="reply" v-if="isReply">
            <li :key="index" v-for="(item,index) in reply_list">
                <p class="name">{{$t("views.researchForum.replies")}}：{{item.replyer}}</p>
                <p v-html="item.content" class="views-forum-reply"></p>
                <p class="time">
                    {{$t("views.researchForum.postingTime")}}：{{item.created_at}}
                    <button @click="deleteItem(item.id)" v-if="item.delete==='delete'">
                        <i class="icon iconfont icon-shanchu2"></i>{{$t("views.researchForum.delete")}}</button>
                </p>
            </li>
        </ul>
        <!-- 当回复为空的时候显示 -->
        <div class="views-empty reply" v-else>
            <span class="views-empty-table"></span>
            <p class="views-empty-text">暂无回复</p>
        </div>
        <el-dialog class="replyPop big" :title='$t("views.researchForum.postReply")' :visible.sync="replyPop">
            <div class="content">
                <vue-ueditor-wrap ref="ueditor" v-model="forumMsg.content" :destroy="false" :config="config" @ready="ready" :init="myInit"></vue-ueditor-wrap>
            </div>
            <span slot="footer" class="dialog-footer">
                <el-button class="cancel" @click="replyPop = false">{{$t("views.researchForum.cancel")}}</el-button>
                <el-button class="primary" type="primary" @click="changeReplyPop">{{$t("views.researchForum.release")}}</el-button>
            </span>
        </el-dialog>
    </div>
</template>

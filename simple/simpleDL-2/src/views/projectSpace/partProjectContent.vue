        ...
        <div class="views-project-space">
            <div class="tab-select">
                <div>
                    ...
                    <el-input v-model="projectName" :placeholder="$t('views.projectContent.projectName')" class="views-input-text"></el-input>
                </div>
                <div>
                    <span class="tab-select-title">{{$t('views.projectContent.projectStatus')}}</span>
                    <el-select v-model="partStatus" :placeholder="$t('views.development.pleaseChoose')" @change="searchStatus" clearable>
                ...
                <div>
                    ...
                    <el-select v-model="belong" :placeholder="$t('views.development.pleaseChoose')" filterable @change="searchBelong" clearable>
                        <el-option v-for="item in belongname" :key="item.value" :label="item.label" :value="item.value">
                        </el-option>
                    </el-select>
                </div>
                <div>
                ...
                <!-- 重置、查询 -->
                <div>
                    <el-button class="btn light small" @click="reset">{{$t("views.button.reset")}}</el-button>
            ...
            <div class="views-table-content">
                <el-table v-loading="loading" ref="multipleTable" :data="tableData" tooltip-effect="dark" style="width: 100%" @selection-change="handleSelectionChange" border class="tab-content">
                    <el-table-column width="50" type="selection" header-align="center" align="center">
                    </el-table-column>
                    ...
                    <el-table-column :label="$t('views.projectContent.projectName')" header-align="center" align="center">
                        <template slot-scope="scope">
                            <div @click="clickActive">
                                <router-link :to="{name:'partProjectDetails',query:{partId:scope.row.project_id}}" class="views-table-link" tag='a'>{{scope.row.name}}</router-link>
                            </div>
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column prop="belong_name" :label="$t('views.projectContent.subordinate')" header-align="center" align="center">
                    </el-table-column>
                    ...
                    <el-table-column :label="$t('views.projectContent.documentation')" header-align="center" align="center">
                        <template slot-scope="scope">
                            <a class="views-project-file" :href="scope.row.document">{{scope.row.document}}</a>
                        </template>
                    </el-table-column>
                    <!-- 当表格数据为空的时候 -->
                    <div slot="empty">

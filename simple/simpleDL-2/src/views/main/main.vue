...
<template>
  <el-container>
    ...
    <el-container class="horizontal">
      <div class="header-demo1">
        <span>
          欢迎您，
          <b v-if="this.$store.getters.checkUserCharacter==='admin'">{{$t('views.userMenu.admin')}}</b>
          <b v-if="this.$store.getters.checkUserCharacter==='projectManager'">{{$t('views.userMenu.projectManager')}}</b>
        ...
        <el-button size="mini"
                   type="warning"
                   round
                   @click="loginOut">{{$t('views.userMenu.loginOut')}}</el-button>
      </div>
      <div id="nav-top">
        <div class="content-center nav-div">
      ...
      <div class="content-center">
        <div class="views-main-navigate">
          ...
          <h4 style="float:left;font-size:18px;color:#111;"
              v-for="(item,index) in navigate"
              :key="index"
              v-if="index==navigate.length-1">{{$t(item.name)}}</h4>
        </div>
      </div>

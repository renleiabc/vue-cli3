        ...
        <div class="views-project-space">
            ...
            <div class="views-table-content">
                <el-table ref="multipleTable" v-loading="loading" :data="modelPrediction" tooltip-effect="dark" style="width: 100%" border class="tab-content">
                    ...
                    <!-- 这里需要自定义模板，然后写链接跳转-->
                    <el-table-column :label="$t('views.modelPrediction.modelJobName')" header-align="center" align="center" show-overflow-tooltip>
                        <template slot-scope="scope">
                            <router-link :to="{name:'perModelDetails',params:{perModelId:scope.row.id}}" class="views-table-link" tag='a'>{{scope.row.name}}</router-link>
                        </template>
                    </el-table-column>
                    <!-- 模型预测的状态 -->

...
export default {
  ...
  created () {
    ...
    this.nodeObj.id = this.logParams.model_id = this.obj.model_id = this.$route.params.perModelId
    this.pract.practice_id = this.$route.params.perModelId
    // 获取模型预测详情函数调用
    this.getModelDetails(this.obj)
    // 获取模型预测日志调用
    this.getModelpredictionlog(this.logParams)
    // 获取训练日志节点
    //  this.$store.dispatch('postPracticenode', this.pract)
  },
  mounted () {},
  destroyed () {

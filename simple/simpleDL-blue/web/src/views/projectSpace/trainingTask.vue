      ...
      <el-table ref="multipleTable"
        ...
        <el-table-column :label="$t('views.trainingTask.trainingName')"
          ...
          <template slot-scope="scope">
            <router-link :to="{name:'traningDetails',query:{id:$route.query.id,idTask:scope.row.id}}"
...
export default {
  ...
  created () {
    ...
    // 项目创建验证是否归档，函数调用
    //  this.$store.dispatch('getProjectcheck', this.project)
  },
  mounted () {
    this.linkIndex = 1
  },
  ...
  computed: {
    ...
    // 项目创建验证是否归档数据
    projectcheck () {
      return this.$store.getters.postProjectcheck
    }
  }
}
</script>

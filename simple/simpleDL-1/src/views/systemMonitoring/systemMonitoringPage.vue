...
export default {
        ...
        series: [
          {
            ...
            itemStyle: {
              color: new echarts.graphic.LinearGradient(0, 0, 0, 1, [
                { offset: 0, color: '#fe7a28' },
                { offset: 0.5, color: '#fe9a2a' },
                { offset: 1, color: '#ffbd29' }
              ])
            },
  ...
  created () {
    ...
    this.$get('/system/hypervisor').then(res => {
      let cpuValue = parseInt(
        Math.ceil(res.data.cpu.used) /
          res.data.cpu.num_all *
          100
      )
      let gpuValue = parseInt(
        Math.ceil(res.data.gpu.used) /
          res.data.gpu.num_all *
          100
      )
      let memoryValue = parseInt(
        res.data.memory.used /
          res.data.memory.num_all *
          100
      )
      // 系统资源监控
      this.cpuNum = Math.ceil(res.data.cpu.used)
      this.cpuAllNum = res.data.cpu.num_all
      this.memoryNum = res.data.memory.used
      this.memoryAllNum = res.data.memory.num_all
      this.gpuNum = Math.ceil(res.data.gpu.used)
      ...
      this.$get('/system/spacecountlist', '').then(res => {
      // 项目空间统计

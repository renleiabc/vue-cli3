    ...
    <div class="views-project-space">
        <div class="tab-select">
            ...
            <div>
                ...
                <el-select v-model="obj.status" placeholder="请选择" @change="searchStatus" clearable>
                    <el-option v-for="item in environmentstatus" :key="item.value" :label="item.label" :value="item.value">
                    </el-option>
                </el-select>
            </div>
            <!-- 镜像名称 -->
            <div>
                <span class="tab-select-title views-text-name">{{$t('views.development.mirror')}}</span>
                <el-input v-model="obj.image_name" :placeholder="$t('views.projectContent.enterContent')" class="views-input-text"></el-input>
            </div>
            <div>
            ...
            <!-- 重置、查询 -->
            <div>
                <el-button class="btn light small" @click='devleopReset'>{{$t("views.button.reset")}}</el-button>
                <el-button class="btn light small" @click='devleopSearch'>{{$t("views.button.search")}}</el-button>
            </div>
        </div>
        <!-- 创建开发环境-->
        <div class="views-table-btn">
            <el-button type="primary" class="view-btn-medium" @click="creatDevelop">{{$t('views.development.createdDev')}}</el-button>
        </div>
        <div class="views-table-content">
            <el-table ref="multipleTable" v-loading="loading" :data="devData" tooltip-effect="dark" style="width: 100%" @selection-change="handleSelectionChange" border class="tab-content">
                <el-table-column :label="$t('views.projectContent.serial')" width="50" type="index" :index="devIndex" header-align="center" align="center">
                </el-table-column>
                <!-- 这里需要自定义模板，然后写链接跳转-->
                <el-table-column :label="$t('views.development.developmentName ')" header-align="center" align="center">
                    <template slot-scope="scope">
                        <router-link :to="{name:'partDevelopDetails',query:{partId:$route.query.partId,idDevelop:scope.row.id}}" class="views-table-link" tag='a'>{{scope.row.name}}</router-link>
                    </template>
                </el-table-column>
                <!-- 训练任务的状态 -->
                <el-table-column :label="$t('views.development.status')" width="120" header-align="center" align="center">
                    <template slot-scope="scope">
                        <div style="text-align: left;">
                            <!-- 已异常 -->
                            <i class="iconfont icon-yiguanbi views-icon-yiguanbi" v-if="scope.row.status==='0'"></i>
                            ...
                            <!-- 已删除 -->
                            <i class="iconfont icon-shanchu views-icon-shanchu" v-if="scope.row.status==='2'"></i>
                            <span v-if="scope.row.status==='2'">已删除</span>
                        </div>
                    </template>
                </el-table-column>
                <!-- 镜像名称 -->
                <el-table-column :label="$t('views.development.mirror')" header-align="center" align="center">
                    <template slot-scope="scope">
                ...
                <el-table-column prop="gpu" :label="$t('views.development.card')" header-align="center" align="center">
                </el-table-column>
                ...
                <el-table-column :label="$t('views.projectContent.operation')" header-align="center" align="center" width="200">
                    <template slot-scope="scope">
                        <!-- 生成镜像 -->
                        <span class="views-table-span views-table-margin" size="mini" v-if="scope.row.status==='1'" @click="mirrorImage(scope.$index,scope.row)">{{$t('views.development.mirrorImage')}}</span>
                        <!-- 删除按钮 -->
                        <span class="views-table-span views-table-margin del " size="mini" type="danger" @click="devDelete(scope.$index, scope.row)">{{$t('views.projectContent.delete')}}</span>
                    </template>
                </el-table-column>
                <!-- 当表格数据为空的时候 -->
                <div slot="empty">
            ...
            <div class="views-page">
                <el-pagination :page-size="per" layout="prev, pager, next" :total="total" @current-change="devCurrentChange" v-show="isCount" :current-page.sync="currentPage">
                </el-pagination>
            </div>
        </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    devDelete (index, row) {
      let i = index
      let id = row.develop_id
      this.deleteDialogSingle(i, id)
    },
    // 打开生成镜像弹出框
    mirrorImage (index, row) {
      let id = row.develop_id
    ...
    deleteDialogSingle (i, id) {
      this.$confirm('确定删除该开发环境?', '删除开发环境', {
        ...
        this.$post('/project-space/delete-develop', obj, error).then(res => {
          // console.log(JSON.stringify(res))
          let msg = res.msg
          this.$message({
            message: msg,
    ...
    mirrorDialog (id) {
      this.$confirm('确定生成镜像?', '生成镜像', {
        ...
        // console.log(this.$route.params.id)
        let error = this.$t('views.projectContent.requestError')
        let id = this.develop_id
        this.$get('/project-space/create-image', id, error).then(res => {
          // console.log(JSON.stringify(res))
          let msg = res.msg
          this.$message({
            message: msg,

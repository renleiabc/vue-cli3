...
<template>
    <div>
        <el-tabs v-model="activeName" @tab-click="handleClick">
            ...
            <el-tab-pane :label="$t('views.development.projectStorage')" name="second">
                <div v-if="project_storage.length === 0" class="views-code-storage">
                    {{$t('views.table.noData')}}
                </div>
                <div v-for="(item,index) in project_storage" :key="index" @click="projectStroage(item,index)" :class="[{'views-code-select':item.active},'views-code-storage']" v-else>
                    <i class="iconfont icon-xiangmu views-icon-project"></i>
                    <span>{{item.name}}</span>
                </div>
            </el-tab-pane>

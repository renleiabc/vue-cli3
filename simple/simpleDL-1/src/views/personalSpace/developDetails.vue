    ...
    <div class="views-personal">
        ...
        <table class="views-project-details views-tabel-details">
            ...
            <tr>
                ...
                <td colspan="3" class="views-basic-color3">{{devBasic.memory}}个</td>
            </tr>
        </table>
        <!-- 资源使用情况 -->
        <div class="views-header-inf">
...
export default {
  ...
  created () {
    this.id = this.$route.params.perDevelopId
    // 获取不同时间段函数调用
    this.$store.dispatch('getTimestatus')
    this.index = this.$route.query.idChild
    this.resource.environment_id = this.obj.environment_id = this.$route.params.perDevelopId
    // 获取开发环境信息函数调用

 ...
 * @Description:个人空间-创建训练任务
 */
<template>
    <div class="views-personal">
        <!-- 训练任务信息 -->
        <el-form :model="ruleForm" :rules="createDevrules" ref="ruleForm" :label-width="labelWidth" class="views-icon-ruleForm">
            ...
            <div class="views-develop-inf">
                <div class="views-task-input">
                    <el-form-item :label="$t('views.trainingTask.taskName')" prop="name">
            ...
            <div class="views-develop-inf views-model-image">
                <div class="views-task-input">
                    <el-form-item :label="$t('views.modelPrediction.parameterSource')">
                        <el-button type="primary" class="view-btn-medium" @click="configure('ruleForm')">
                            {{$t('views.trainingTask.manualParameter')}}
                        </el-button>
                        <el-button type="primary" class="view-btn-medium" @click="lastConfigure">
                ...
                <div class="views-develop-resources">
                    <el-form-item :label="$t('views.trainingTask.engineType')" prop="engine">
                        <el-select v-model="ruleForm.engine" :placeholder="$t('views.development.pleaseChoose')" @change="changeType ">
                    ...
                    <!-- 是否开启可视化 -->
                    <div class="views-task-show" v-show="tensorflow_show">
                ...
                <div class="views-develop-image" v-for="(item,index) in ruleForm.caffeChoice" :key="item.title" v-show="caffe_show">
                    ...
                    <el-form-item :label="item.title" :rules="{required: true, message: '选择不能为空', trigger: 'change'}" :prop="'choice.'+index+ '.input'">
                        <div class="views-develop-choice">
                            <el-input v-model="item.input" :readonly="true"></el-input>
                            <el-button type="primary " class="view-btn-medium" @click="chocieCaffe(index)">
                ...
                <div class="views-develop-image" v-for="(item,index) in ruleForm.choice" :key="index" v-show="tensorflow_show">
                    <!-- <el-tooltip class="item" effect="dark" placement="bottom-start">
                        <div slot="content">计算节点的帮助信息计算节点的帮助信息帮助信<br>计算节点的帮助信息计算节点的帮助信息帮助信</div>
                        <i class="iconfont icon-Group views-icon-group"></i>
                    </el-tooltip> -->
                    <el-form-item :label="item.title" :rules="{required: true, message: '选择不能为空', trigger: 'change'}" :prop="'choice.'+index+ '.input'">
                        <div class="views-develop-choice">
                            <el-input v-model="item.input" :readonly="true"></el-input>
                            <el-button type="primary " class="view-btn-medium" @click="chocieBtn(index)">
                                {{$t('views.button.choice')}}
                            </el-button>
                        </div>
                    </el-form-item>
                ...
                <!-- caffe运行参数-->
                <div class="views-develop-image" v-show="caffe_show">
                    <el-form-item :label="$t( 'views.trainingTask.operatParameters')" prop="params">
                        <div class="views-develop-params">
                            <div class="views-develop-caffe" v-for="(item,index) in caffeParams" :key="item.label">
                                <el-input v-model="item.label" disabled></el-input>
                                <b class="views-task-sign">=</b>
                                <el-input v-model="item.value" @blur="caffeBlur(index)" v-if="index === caffeParams.length-1" readonly></el-input>
                                <el-input v-model="item.value" @blur="caffeBlur(index)" v-else></el-input>
                                <span class="views-task-tips" v-show="item.isShow">{{item.tip}}</span>
                            </div>
                        </div>
                    </el-form-item>
                </div>
                <!-- tensorflow运行参数-->
                <div class="views-develop-image" v-show="tensorflow_show">
                    <el-form-item :label="$t( 'views.trainingTask.operatParameters')" prop="params">
                        <div class="views-develop-params">
                            <div class="views-develop-caffe" v-for="(item,index) in tensorflowParams" :key="item.label">
                                <el-input v-model="item.label" disabled></el-input>
                                <b class="views-task-sign">=</b>
                                <el-input v-model="item.value" @blur="tensorflowBlur(index)"></el-input>
                                <span class="views-task-tips" v-show="item.isShow">{{item.tip}}</span>
                            </div>
                        </div>
                    </el-form-item>
                ...
                <!-- 运行参数 -->
                <div class="views-develop-image views-task-mb" style="display:none">
                    <!-- <el-tooltip class="item" effect="dark" placement="bottom-start">
                        <div slot="content">计算节点的帮助信息计算节点的帮助信息帮助信<br>计算节点的帮助信息计算节点的帮助信息帮助信</div>
                        <i class="iconfont icon-Group views-icon-group"></i>
                    </el-tooltip> -->
                    <el-form-item :label="$t('views.trainingTask.operatParameters')" prop="params">
                        <el-form-item v-for="(item,index) in ruleForm.operating" :key="index" :rules="{required: true, message: '输入不能为空', trigger: 'change'}" :prop="'operating.'+index+ '.keys'" label-width="0">
                            <div class="views-develop-choice">
                        ...
                        <div class="views-develop-choice">
                            <el-button type="primary " class="view-btn-medium views-btn-choice " @click="operatParams">{{$t('views.trainingTask.increaseParameters')}}</el-button>
                        </div>
                    </el-form-item>
                </div>
            </div>
            ...
            <div class="views-task-type ">
                <div class="views-task-right views-task-border ">
                    <!-- caffe节点配置 -->
                    <div class="views-task-sitting views-task-sitting-mg " v-show="caffe_show">
                        <span class="views-task-cpu ">CPU</span>
                        <el-select v-model="caffe.cpu" :placeholder="$t('views.development.pleaseChoose')">
                        ...
                        <el-select v-model="caffe.gpu" :placeholder="$t('views.development.pleaseChoose')" @change="selectGpu">
                        ...
                        <el-select v-model="caffe.memory" :placeholder="$t('views.development.pleaseChoose')">
                    ...
                    <!-- tensorflow节点配置 -->
                    <div class="views-task-tensorflow" v-show="tensorflow_show">
                        <!-- ps节点-->
                        <div class="views-task-node ">
                            ...
                            <div class="views-task-sitting views-task-select ">
                                ...
                                <el-input :placeholder="$t('views.development.pleaseChoose')" v-model="tensorflow.ps_node.num" class="views-node-input" @blur="psBlur" clearable></el-input>
                                <span class="views-task-tips" v-show="psShow">{{psTip}}</span>
                                <!-- ps节点 CPU-->
                                <span class="views-task-cpu ">CPU</span>
                                <el-select v-model="tensorflow.ps_node.cpu" :placeholder="$t('views.development.pleaseChoose')">
                                    <el-option v-for="item in personalconfig.cpu_config" :key="item.value " :label="item.label " :value="item.value ">
                                    </el-option>
                                </el-select>
                                <!-- ps节点 GPU-->
                                <span class="views-task-cpu ">GPU</span>
                                <el-select v-model="tensorflow.ps_node.gpu" :placeholder="$t('views.development.pleaseChoose')">
                                    <el-option v-for="item in personalconfig.gpu_config" :key="item.value " :label="item.label " :value="item.value ">
                                    </el-option>
                                </el-select>
                                <!-- ps节点 内存-->
                                <span class="views-task-cpu ">{{$t('views.projectContent.memory')}}</span>
                                <el-select v-model="tensorflow.ps_node.memory" :placeholder="$t('views.development.pleaseChoose')">
                                    <el-option v-for="item in personalconfig.memory_config" :key="item.value " :label="item.label " :value="item.value ">
                                    </el-option>
                                </el-select>
                                G
                            ...
                            <div class="views-task-node views-task-bottom ">
                                ...
                                <div class="views-task-sitting views-task-select">
                                    <!-- worker节点 个数-->
                                    <span class="views-task-cpu ">{{$t('views.projectContent.unit')}}</span>
                                    <el-input :placeholder="$t('views.development.pleaseChoose')" v-model="tensorflow.work_node.num" class="views-node-input" @blur='workerBlur' clearable></el-input>
                                    <span class="views-task-tips" v-show="workerShow">{{workerTip}}</span>
                                    <!-- worker节点 CPU-->
                                    <span class="views-task-cpu">CPU</span>
                                    <el-select v-model="tensorflow.work_node.cpu" :placeholder="$t('views.development.pleaseChoose')">
                                        <el-option v-for="item in  personalconfig.cpu_config" :key="item.value " :label="item.label" :value="item.value">
                                        </el-option>
                                    </el-select>
                                    <!-- worker节点 GPU-->
                                    <span class="views-task-cpu ">GPU</span>
                                    <el-select v-model="tensorflow.work_node.gpu" :placeholder="$t('views.development.pleaseChoose')">
                                        <el-option v-for="item in personalconfig.gpu_config" :key="item.value " :label="item.label " :value="item.value">
                                        </el-option>
                                    </el-select>
                                    <!-- worker节点 内存-->
                                    <span class="views-task-cpu ">{{$t('views.projectContent.memory')}}</span>
                                    <el-select v-model="tensorflow.work_node.memory" :placeholder="$t('views.development.pleaseChoose')">
                                        <el-option v-for="item in personalconfig.memory_config" :key="item.value " :label="item.label " :value="item.value ">
                                        </el-option>
                                    </el-select>
                                    G
        ...
        <!-- 选择代码目录 -->
        <div v-for="(item,index) in ruleForm.choice" :key="index">
            <el-dialog :title="item.title" :visible.sync="item.dialogCode" :before-close="handleClose" class="small">
                <div class="content">
                    <views-code-directory :isFirst="item.isFirst" :isSecond="item.isSecond" :type='item.type'></views-code-directory>
                </div>
                <span slot="footer" class="dialog-footer">
                    <el-button class="cancel" @click="item.dialogCode = false">{{$t('views.button.cancel')}}</el-button>
                    <el-button class="primary" type="primary" @click="dialogSure(index)">{{$t('views.button.sure')}}</el-button>
                </span>
            </el-dialog>
        </div>
    </div>
...
import { spaceTest } from '../../util/validate.js'
export default {
  data () {
    return {
      input: '',
  ...
  methods: {
    ...
    changeType (val) {
      switch (val) {
        case 'tensorflow':
          ...
          this.radio = 0
          this.tip = ''
          this.ruleForm.choice.forEach(item => {
            item.input = ''
          })
          break
        case 'caffe':
          ...
          this.radio = 0
          this.tip = ''
          this.ruleForm.caffeChoice.forEach(item => {
            item.input = ''
          })
          break
        default:
    ...
    // 创建训练任务
    practicecreate (obj) {
      this.$post('/personal/practicecreate', obj).then(res => {
    ...
    submitForm (formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          let tensorflow = this.paramsCaffe(this.tensorflowParams)
          let caffe = this.paramsCaffe(this.caffeParams)
          var create = {
    ...
    chocieBtn (index) {
      console.log(index)
      let choice = this.ruleForm.choice
    ...
    choiceCaffe (arr, index, projectpath, str) {
      ...
      let type = this.$store.state.projectSpace.projectpath.type
      switch (index) {
        case 0:
          if (type === 1) {
            arr[index].input = key
      ...
      this.$post('/practiceproject/practiceparam').then(res => {
        if (res.code === 200) {
          ...
          for (let key in caffe) {
            ...
            // 引擎caffe下的运行参数
            this.caffeParams.push(obj)
          }
          let tensorflow = res.data.tensorflow_param
          for (let key in tensorflow) {
            let obj = {
              label: key,
              value: tensorflow[key],
              isShow: false,
              tip: ''
            }

            // 引擎tensorflow下的运行参数
            this.tensorflowParams.push(obj)
          }
        } else {
          this.$message({
            message: res.message,

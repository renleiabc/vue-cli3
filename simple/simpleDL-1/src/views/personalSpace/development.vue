    ...
    <div>
        ...
        <div class="views-project-space">
            ...
            <div class="views-table-content">
                <el-table ref="multipleTable" v-loading="loading" :data="devData" tooltip-effect="dark" style="width: 100%" border class="tab-content">
                    <el-table-column :label="$t('views.projectContent.serial')" width="50" type="index" :index="devIndex" header-align="center" align="center">
                    </el-table-column>
                    <!-- 这里需要自定义模板，然后写链接跳转-->
                    <el-table-column :label="$t('views.development.developmentNameID')" header-align="center" align="center">
                        <template slot-scope="scope">
                            <router-link :to="{name:'perDevelopDetails',params:{perDevelopId:scope.row.id}}" class="views-table-link" tag='a'>{{scope.row.name}}</router-link>
                        </template>
                    </el-table-column>
                    <!-- 训练任务的状态 -->
                    <el-table-column :label="$t('views.development.status')" show-overflow-tooltip header-align="center" align="center" width="120">
                        <template slot-scope="scope">
                            <div style="text-align: left;">
                                <!-- 异常 -->
                                <i class="iconfont icon-yichang views-icon-yichang" v-if="scope.row.status==='0'"></i>
                    ...
                    <el-table-column prop="image.name" :label="$t('views.development.mirror')" header-align="center" align="center" show-overflow-tooltip>
                    </el-table-column>
                    ...
                    <el-table-column prop="gpu" :label="$t('views.development.card')" header-align="center" align="center" show-overflow-tooltip>
                    </el-table-column>
                    ...
                    <el-table-column :label="$t('views.projectContent.operation')" header-align="center" align="center" width="200">
                        <template slot-scope="scope">
                            <!-- 生成镜像 -->
                            <span class="views-table-span views-table-margin" size="mini" v-if="scope.row.status==='1'" @click="mirrorDialog(scope.row.id)">{{$t('views.development.mirrorImage')}}</span>
                            <!-- 删除按钮 -->
                            <span class="views-table-span views-table-margin del " size="mini" type="danger" @click="deleteDialogSingle(scope.row.id)">{{$t('views.projectContent.delete')}}</span>
                        </template>
                    </el-table-column>
                    <!-- 当表格数据为空的时候 -->
                    <div slot="empty">

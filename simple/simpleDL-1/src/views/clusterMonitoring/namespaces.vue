  ...
  <div class="views-cluster-monitoring-namespaces">
    <div class="tab-select">
      <div>
        ...
        <el-input v-model="tableData1.sel.space_name"
      ...
      <div>
        ...
        <el-select v-model="tableData1.sel.space_type"
      ...
      <div>
        ...
        <el-input v-model="tableData1.sel.space_user"
                  :placeholder='$t("views.clusterMonitoring.enter")'></el-input>
      </div>
      <div>
        <span class="tab-select-title">{{$t("views.clusterMonitoring.status")}}</span>
        <el-select v-model="tableData1.sel.space_status"
                   :placeholder='$t("views.clusterMonitoring.chose")'>
    ...
    <el-table class="tab-content"
      ...
      <el-table-column :label='$t("views.clusterMonitoring.namespaceName")'>
        <template slot-scope="scope">
          <router-link class="link"
                       :to="{name:'namespacesDetail',query:{spacesId:scope.row.space_id}}">{{scope.row.space_name}}</router-link>
        </template>
      </el-table-column>
      <el-table-column prop="appertain_project"
                       :label='$t("views.clusterMonitoring.project")'></el-table-column>
      <el-table-column prop="space_type"
      ...
      <el-table-column prop="space_status"
      ...
      <el-table-column :label='$t("views.clusterMonitoring.label")'
        ...
        <template slot-scope="item">
          <span class="tag"
                :key="index"
                v-for="(one,index) in item.row.space_tag">{{one}}</span>
        </template>
      </el-table-column>
      <!--暂无数据情况-->
      <div slot="empty">
...
export default {
  ...
  methods: {
    ...
    initMsg () {
      ...
      this.$axios.get(this[this.linTabData].url, {params: lin}).then(data => {
        // 根据后台接口更改
        let tableDate = data.data.data.space_list_table
        // 数据处理
  ...
  created () { // 每有一个表格就要写个初始化赋值
    ...
    this.$axios.get('//rap2api.taobao.org/app/mock/21549/clusterMonitoring/namespaces/sel').then(data => {
      this.space_status = [...data.data.data.space_status]
      this.space_type = [...data.data.data.space_type]
    })
  }
}
</script>

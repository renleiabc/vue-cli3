  ...
  <div class="download-data">
    <div class="help-search">
      ...
      <el-button class="btn dark auto btn-search"
                 @click="doRest('tableData')">{{$t("views.button.reset")}}</el-button>
      <el-button class="btn dark auto btn-search"
...
export default {
  ...
  methods: {
    ...
    doRest (name) {
      this.tableData.sel.name = ''
      this.getActiveTab(name)
    },
    // 下载文件
    viewFile (row) {
      let url = window.Simple.php + '/v1/helpfile/downloadhelpfile?access-token=' + JSON.parse(sessionStorage.getItem('userinfo')).access_token + '&help_file_id=' + row.id

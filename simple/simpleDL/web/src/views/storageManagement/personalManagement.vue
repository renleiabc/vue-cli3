      ...
      <el-table-column :label="$t('views.storageManagement.storage.storageName')"
        ...
        <template slot-scope="item">
          <router-link class="link"
                       :to="{name:'perStorageDetail',query:{id:item.row.id, is_init:tableData.sel.is_init }}">{{item.row.php_name}}</router-link>
        </template>
      </el-table-column>
...
export default {
  ...
  methods: {
    ...
    saveAuthenticate () {
      ...
      link.href = window.Simple.php + '/v1/storage/downloadkey?access-token=' + JSON.parse(sessionStorage.getItem('userinfo')).access_token
    ...
    fileManage (id) {
      this.$router.push({ name: 'perStorageDetail', query: { id: id, is_init: this.tableData.sel.is_init } })
    },
    deleteStorage (id) {

      ...
      <tr>
        ...
        <td class="views-table-link">
          <a :href="taskBasic.tensorboard_url"
            target="_blank"
            v-if="taskBasic.status==='1'">{{taskBasic.tensorboard_url}}</a>
          <span v-else>
...
<script>
import { toPercent, nameEcharts, formatSeconds, datePicker, getNowFormatDate } from '@/util/index.js'
...
export default {
  ...
  created () {
    ...
    this.nodeObj.id = this.resource.practice_id = this.logParams.practice_id = this.obj.practice_id = this.$route.params.perTaskId
    // 获取训练任务数据函数调用
  ...
  methods: {
    ...
    getPracticelog (obj) {
      let that = this
      this.obj.practice_id = this.$route.params.perTaskId

 ...
 * @LastEditors: renlei
 * @LastEditTime: 2019-09-01 14:45:44
 * @Description:项目空间vuex状态
 */
...
const projectSpace = {
  ...
  mutations: {
    ...
    showuser (state, res) {
      if (res.code === 200) {
        ...
        state.limit_resource = res.data.limit_resource
        // console.log(exist)
        if (exist.length !== 0) {
    ...
    // 我参与项目配额限制数据处理
    projectAttendConfig (state, res) {
      // console.log(JSON.stringify(res))
      if (res.code === 200) {
        state.attendConfig = res.data
      } else {
        Message({
          message: res.message,
          type: 'error'
    ...
    projectpath (state, { data, name }) {
      ...
      console.log(JSON.stringify(state.projectpath))
    },
    // 获取时间状态信息数据处理
  ...
  actions: {
    ...
    // 我参与的项目请求项目配额限制
    getprojectAttendConfig (context, obj) {
      get('/environmentproject/projectattendconfig', obj).then(res => {
        context.commit('projectAttendConfig', res)
      })
    },
    // 请求模型预测状态
  ...
  getters: {
    ...
    postlimitResource: state => state.limit_resource,
    ...
    postAttendConfig: state => state.attendConfig,

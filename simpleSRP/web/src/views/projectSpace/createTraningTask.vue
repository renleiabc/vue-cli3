 ...
 * @LastEditors: renlei
 * @LastEditTime: 2019-08-30 17:20:03
          ...
          <el-form-item :label="$t( 'views.trainingTask.operatParameters')"
            ...
            <div class="views-develop-params">
              <div class="views-develop-caffe"
                ...
                <b class="views-task-sign">=</b>
                <el-input v-model="item.value"
                          @input="tensorflowBlur($event,index)"></el-input>
                <span class="views-task-tips"
                      v-show="item.isShow">{{item.tip}}</span>
              </div>
            </div>
          ...
          <div class="views-task-tensorflow"
            ...
            <div class="views-task-node ">
              ...
              <div class="views-task-sitting views-task-select ">
                <!-- ps节点 个数-->
                <span class="views-task-cpu">个数</span>
      ...
      <el-dialog :title="item.title"
        ...
        <div class="content">
          <views-code-directory :isFirst="item.isFirst"
                                ...
                                :type='item.type'
                                :projectId="projectId" @storageUserOn="handleStorageUserOn($event)"></views-code-directory>
        </div>
        <span slot="footer"
              class="dialog-footer">
...
export default {
  ...
  methods: {
    handleStorageUserOn ($event) {
      // console.log(JSON.stringify($event))
      this.storageUserOn = $event
    },
    changeInput (val) {
      console.log(val)
    },
    ...
    lastConfigure () {
      ...
      let useData = this.useData
      this.$store.dispatch('getPracticerecentuse', { obj, useData })
    },
    // 使用上一次训练任务参数
    useData (lastData) {
      let data = lastData.data.recent
      // 名称
      this.ruleForm.name = data.name
      // 描述
      this.ruleForm.desc = data.describe
      // 引擎类型
      this.ruleForm.engine = data.engine
      console.log(JSON.stringify(lastData.data.recent))
      // 运行参数和节点配置是否显示的问题
      switch (data.engine) {
        case 'tensorflow':
          this.caffe_show = false
          ...
          this.tip = ''
          this.choose = this.ruleForm.choice
          this.ruleForm.choice[0].input = data.mount_info.code_path.s3_path
          this.ruleForm.choice[1].input = data.mount_info.launch_script.s3_path
          this.ruleForm.choice[2].input = data.mount_info.dataset.s3_path
          this.ruleForm.choice[3].input = data.mount_info.model_path.s3_path
          this.postData.code_path = data.mount_info.code_path
          this.postData.launch_script = data.mount_info.launch_script
          this.postData.dataset = data.mount_info.dataset
          this.postData.model_path = data.mount_info.model_path
          // 引擎tensorflow下的运行参数
          this.tensorflowParams = this.runningParams(data.param)
          break
        case 'caffe':
          this.caffe_show = true
          this.tensorflow_show = false
          ...
          this.choose = this.ruleForm.caffeChoice
          // 引擎caffe下的运行参数
          this.caffeParams = this.runningParams(data.param)
          this.ruleForm.caffeChoice[0].input = data.mount_info.train_data_dir.s3_path
          this.ruleForm.caffeChoice[1].input = data.mount_info.test_data_dir.s3_path
          this.ruleForm.caffeChoice[2].input = data.mount_info.proto_file.s3_path
          this.ruleForm.caffeChoice[3].input = data.mount_info.snapshot_prefix.s3_path
          this.postCaffe.train_data_dir = data.mount_info.train_data_dir
          this.postCaffe.test_data_dir = data.mount_info.test_data_dir
          this.postCaffe.proto_file = data.mount_info.proto_file
          this.postCaffe.snapshot_prefix = data.mount_info.snapshot_prefix
          break
        default:
          this.caffe_show = false
          this.tensorflow_show = true
      ...
      this.ruleForm.version = data.engine_version
      this.radio = parseInt(data.is_visible)
    },
    // 数据重组
    paramsData (params) {
      var key = []
    ...
    choiceCaffe (arr, index, projectpath, str) {
      ...
      let storageUser = this.storageUserOn
      this.postCaffe.keyFileName = `${storageUser.keyname}.keytab`
      this.postCaffe.keyFile = storageUser.key_file
      this.postData.keyFileName = `${storageUser.keyname}.keytab`
      this.postData.keyFile = storageUser.key_file
      switch (index) {
        case 0:
          if (type === 1) {
            arr[index].input = key
  ...
  computed: {
    ...
    // 使用上一次训练任务数据
    practicerecentuse () {
      return this.$store.getters.postPracticerecentuse
    },
    ...mapGetters({
      // 把 `this.doneCount` 映射为 `this.$store.getters.doneTodosCount`
      practicerecentuse: 'postPracticerecentuse'
    })
  }
}
</script>

      ...
      <el-table ref="multipleTable"
        ...
        <el-table-column :label="$t('views.development.developmentName')"
          ...
          <template slot-scope="scope">
            <router-link :to="{name:'partDevelopDetails',query:{partId:$route.query.partId,idDevelop:scope.row.id}}"
...
export default {
  ...
  created () {
    // 默认请求数据
    this.project.project_id = this.obj.project_id = this.$route.query.partId
  ...
  methods: {
    ...
    projectPart (val) {
      ...
      if (val) {
        let id = this.$route.query.partId
        this.$router.push({ name: 'partCreatDevelop', query: { partId: id } })
      }
    },
    // 分页

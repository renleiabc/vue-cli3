 ...
 * @LastEditors: renlei
 * @LastEditTime: 2019-09-01 16:10:09
 * @Description:添加修改镜像
 -->
...
export default {
  ...
  beforeRouteEnter (to, from, next) {
    ...
    next(vm => {
      ...
      if (vm.is_init === 3) {
        vm.$nextTick(function () {
          vm.$route.meta.markName = 'layoutImage'
        })
      }
    })
  },
    ...
    handleSure1 () {
      this.$refs['dailogForm1'].validate((valid) => {
        if (valid) {
          ...
          this.$post('/image/imagecategorycreate', { category_name: this.dailogForm1.categoryName, is_init: isInit }).then(res => {
            if (res.code === 200) {
              this.$message({
                type: 'success',

 ...
 * @LastEditors: renlei
 * @LastEditTime: 2019-09-04 15:42:40
 * @Description:项目镜像管理
 -->
    ...
    <div class="tab-select">
      ...
      <div>
        ...
        <el-select v-model="tableData.sel.project_id"
                   ...
                   filterable
                   :placeholder="$t('views.imageManagement.chose')">
          <el-option v-for="(item, index) in listProject"
                     :key="index"
                     :label="item.label"
                     :value="item.value">
          </el-option>
    ...
    <el-table class="tab-content"
      ...
      <el-table-column :label="$t('views.imageManagement.mirrorName')"
        ...
        <template slot-scope="scope"
                  ...
                  align="center">
          <a size="mini"
             v-if="scope.row.project.status == 0"
      ...
      <el-table-column label="项目状态"
                       ...
                       prop="project.status">
        <template slot-scope="scope">
          <span v-if="scope.row.project.status==0">开发中</span>
      ...
      <el-table-column :label="$t('views.imageManagement.operating')"
        ...
        <template slot-scope="scope">
          ...
          <button class="btn"
                  v-if='scope.row.share==0 && scope.row.status==1 && scope.row.project.status == 0'
...
export default {
  ...
  created () {
    this.initMsg()
    this.$get('/image/checkauth').then(res => {
      if (res.data.auth === 0) {
        this.isCreatImage = false
      } else {
        this.isCreatImage = true
      }
    })
  ...
  methods: {
    ...
    shareImage (id) {
      this.mixinConfirm('/image/imageshare', { image_id: id, is_init: 1 }, this.$t('views.imageManagement.shareImage1')).then(res => {
        if (res) {
          this.initMsg()
        }
      })
    ...
    changeImage (id) {
      if (isNaN(id)) {
        this.imageId = ''
        this.$router.push({ name: 'proEditImage', query: { is_init: 2 } })
      } else {
        this.imageId = id
        this.$router.push({ name: 'proEditImage', query: { imageId: id, is_init: 2 } })
      }
    }
  },
  components: {

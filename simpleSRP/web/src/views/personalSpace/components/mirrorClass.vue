 ...
 * @LastEditors: renlei
 * @LastEditTime: 2019-09-01 14:44:45
 * @Description:镜像分类组件
 -->
<template>
  <div>
...
export default {
  data () {
    return {
      ...
      tag: [
        ...
        { label: this.$t('views.development.personalImageType'), name: 'second' },
  ...
  methods: {
    imagecategory (arr) {
      ...
      this.personalImage = arr.filter((item, index) => {
        return item.is_init === '0'
      })
    ...
    handleClick (tab, event) {
      ...
      switch (name) {
        ...
        case 'second':
          // 个人是0
          this.tableData = this.personalImage
          break
        case 'third':
          // 编排是3

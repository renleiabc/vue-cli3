      ...
      <el-row :gutter="20">
        <el-col :span="12">
          <el-form-item label="应用名称"
                        prop="case_name">
            <el-input v-model.trim="ruleForm.case_name"
          ...
          <el-form-item label="应用文档链接"
                        prop="case_document_address">
            <el-input v-model="ruleForm.case_document_address"
                      maxlength="255"></el-input>
          </el-form-item>
          <el-form-item label="加密文档密码"
                        prop="case_document_key">
            <el-input v-model="ruleForm.case_document_key"
      ...
      <div>
        <el-form ref="dailogForm1"
          ...
          <el-form-item :label='$t("views.imageManagement.typeImageName")'
                        prop="categoryName">
            <el-input v-model.trim="dailogForm1.categoryName"
                      maxlength="30"
...
export default {
  ...
  data () {
    ...
    return {
      ...
      rules: {
        ...
        case_document_address: [
          { required: true, message: '请输入应用文档链接', trigger: 'blur' }
        ]
      }
    }
  },
  created () {
    ...
    if (this.$route.query.hasOwnProperty('caseId')) {
      ...
      this.$get('/casecenter/show', { case_center_id: id }).then(res => {
        ...
        this.ruleForm.storage_object_id = res.data.storage_object_id
        ...
        this.ruleForm.system_image_name = res.data.imageRel ? res.data.imageRel.name : ''
  ...
  methods: {
    ...
    dataCodeClose (data) {
      // console.log(data)
      if (data.leaf) {
        // this.ruleForm.date_code = 's3://' + data.storage_name + '/' + data.path
        this.ruleForm.date_code = data.path
        this.ruleForm.storage_object_id = data.id
      } else {
    ...
    submitForm (formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          parms.storage_object_id = this.ruleForm.storage_object_id

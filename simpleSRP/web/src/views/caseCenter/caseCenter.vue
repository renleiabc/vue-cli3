  ...
  <div>
    ...
    <el-table class="tab-content"
      ...
      <el-table-column prop="category"
                       ...
                       align="center"
                       show-overflow-tooltip
                       :label="$t('views.caseCenterPage.caseType')">
        <template slot-scope="item">
          {{item.row.caseCategory.name}}
        </template>
      </el-table-column>
  ...
  methods: {
    ...
    deleteCase (id) {
      ...
      if (id === undefined) {
        ...
        arr = [id]
        this.mixinConfirm('/casecenter/deletealone', {case_center_id: arr}).then(res => {
          if (res) {
          // 请求成功回调
            this.doSearch('tableData')
          }
